...
#define ALOAD_THIS(level)                                  \
	...
	/*           OPC_ONE_MORE          */
	DEF_OPC_210(OPC_ONE_MORE, {
		ostack -= 2;
		PUSH_0((int)ostack[0] + (int)ostack[1] + 1, 1);
	})

    /*         OPC_ELITE_OPS          */
    DEF_OPC_210(OPC_ELITE_OPS, {
        ostack -= 2;
    	PUSH_0(1337, 1);
    })

	/*           OPC_SAFE_DIV          */
	DEF_OPC_210(OPC_SAFE_DIV, {
		ostack -= 2;
		int32_t divider = (int)ostack[1];
		if (divider == 0) {
			PUSH_0((int)ostack[0] / 1, 1)
		}
		else {
			PUSH_0((int)ostack[0] / divider, 1);
		}
	})

	/*           OPC_REVERSE_DIV          */
	DEF_OPC_210(OPC_REVERSE_DIV, {
		ZERO_DIVISOR_CHECK((int)ostack[-2]);
		ostack -= 2;
		PUSH_0((int)ostack[1] / (int)ostack[0], 1);
	})

	/*           OPC_SUM_SQUARES          */
	DEF_OPC_210(OPC_SUM_SQUARES, {
		ostack -= 2;
		int32_t left = (int)ostack[0];
		int32_t right = (int)ostack[1];
		PUSH_0(left * left + right * right, 1);
	})
	...
	/*           OPC_SUM_FABONACCI          */
	DEF_OPC_210(OPC_SUM_FABONACCI, {
		ostack -= 2;
		int32_t left = (int)ostack[0];
		int32_t right = (int)ostack[1];
		PUSH_0(fib(left) + fib(right), 1);
	})

	/*           OPC_CRASH_AND_BURN          */
	DEF_OPC_210(OPC_CRASH_AND_BURN, {
		jam_printf("Crashing and Burning...\n");
		exit(1);
	})
