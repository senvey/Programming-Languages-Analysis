...
retry:
    ...
    for(pass = 0; pass < 2; pass++) {
        ...
        /* one instruction consists of multiple op_codes */
        for(ins_count = 0, pc = 0; pc < code_len; ins_count++) {
            ...
            switch(opcode) {
                ...
                case OPC_ONE_MORE: case OPC_ELITE_OPS:
                case OPC_SAFE_DIV: case OPC_REVERSE_DIV:
                case OPC_SUM_SQUARES: case OPC_SUM_FABONACCI:
                case OPC_CRASH_AND_BURN:
#ifdef USE_CACHE
                    cache = 1;
#endif
                    pc += 1;
